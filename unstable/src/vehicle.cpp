...
total_frames = parser.get_int("SURFACE", "framesnb", 1);
frame_timer.start(0);
...
accel_power = parser.get_float("PROPERTIES", "accel_power", 1.60);
tire_friction = parser.get_float("PROPERTIES", "tire_friction", 0.05);
air_friction = parser.get_float("PROPERTIES", "air_friction", 0.1);
rot_vel_power = parser.get_float("PROPERTIES", "rotating_power", 1.20);

std::string power_str = parser.get_string("PROPERTIES", "power", "none");
if(power_str == "jump")
...
if(dying)
  ...
  center.x = (int)old_pos.x - screen_area.w/2;
  center.y = (int)old_pos.y - screen_area.h/2;

  center.x += ((int)(pos.x-old_pos.x)/dying_timer.get_total_time()) *
             dying_timer.get_gone();
  center.y += ((int)(pos.y-old_pos.y)/dying_timer.get_total_time()) *
             dying_timer.get_gone();
  }
else
  {
  center.x = (int)pos.x - screen_area.w/2;
  center.y = (int)pos.y - screen_area.h/2;
  }
...
Pointf Vehicle::get_vehicle_reference_velocity() const
  {
  Pointf vel_vr;
...
Pointf Vehicle::translate_to_world_reference(const Pointf& vel_vr) const
  {
  return Pointf((cos_angle * vel_vr.y) + (sin_angle * vel_vr.x),
...
sin_angle = - sin((int)angle);
cos_angle = - cos((int)angle);

// translate world reference variables to vehicle reference
Pointf vel_vr, accel_vr;
...
terrain_type = Race::current->track->get_terrain_type(Rect(pos, size));
...
//Screen::fill_rect(Rect(pos-scroll,size), Color(0,0,255,128));
...
for(int a = 0; a < 360 / SKIP_ANGLE_RATIO; a++)
  {
  for(int f = 0; f < total_frames; f++)
    surfaces[a][f]->scale(x_scale, y_scale);
  shadows[a]->scale(x_scale, y_scale);
  }
}
